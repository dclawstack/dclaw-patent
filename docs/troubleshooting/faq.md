...
### How do I update DClaw Patent?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp patent --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Patent without Kubernetes?

Yes, for local development:

```bash
cd dclaw-patent/frontend && npm run dev
cd dclaw-patent/backend && uvicorn dclaw_patent.main:app --reload
```
