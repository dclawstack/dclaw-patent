...
# Check events
kubectl get events -n dclaw-patent --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-patent deployment/dclaw-patent-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-patent-db-credentials -n dclaw-patent
   ```
